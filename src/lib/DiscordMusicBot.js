const { Client, Intents, MessageEmbed, Collection, MessageActionRow, MessageButton, Util } = require('discord.js');
const fs = require('fs');
...
class DiscordMusicBot extends Client {
    constructor() {
        console.log('Initializing DiscordMusicBot');
        super({
            intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_VOICE_STATES, Intents.FLAGS.GUILD_MESSAGES]
        });

        this.config = getConfig();
        this.logger = new Logger(path.join(__dirname, '..', 'bot.log'));
    ...
    start() {
        ...
        this.manager = new Manager({
            ...
            .on('nodeConnect', (node) => this.log(`Node: ${node.options.identifier} | Lavalink node is connected.`))
            .on('nodeReconnect', (node) => this.warn(`Node: ${node.options.identifier} | Lavalink node is reconnecting.`))
            .on('nodeDestroy', (node) => this.warn(`Node: ${node.options.identifier} | Lavalink node is destroyed.`))
            .on('nodeDisconnect', (node) => this.warn(`Node: ${node.options.identifier} | Lavalink node is disconnected.`))
            ...
            .on('playerMove', (player, oldChannel, newChannel) => {
                ...
                if (newChannel === null || !newChannel) {
                    ...
                    if (channel) {
                        channel.send({
                            embeds: [new MessageEmbed().setColor(client.config.embedColor).setDescription(`Disconnected from <#${oldChannel}>`)]
                        });
                    }
            ...
            .on('playerCreate', (player) => {
                ...
                this.warn(
                    `Player: ${player.options.guild} | A wild player has been created in ${
            ...
            .on('playerDestroy', (player) => {
                this.warn(
                    `Player: ${player.options.guild} | A wild player has been destroyed in ${
                        client.guilds.cache.get(player.options.guild) ? client.guilds.cache.get(player.options.guild).name : 'a guild'
                    }`
                );
            ...
            .on('loadFailed', (node, type, error) => this.warn(`Node: ${node.options.identifier} | Failed to load ${type}: ${error.message}`))
                ...
                /** @param {EpicPlayer} */ async (player, track) => {
                    ...
                    if (autoQueue) {
                        ...
                        let queueEmbed = new MessageEmbed()
                            ...
                            .setTimestamp();
                        let EndQueue = await client.channels.cache.get(player.textChannel).send({ embeds: [queueEmbed] });
                            ...
                            if (!player.playing && !twentyFourSeven) {
                                setTimeout(async () => {
                                    if (!player.playing && player.state !== 'DISCONNECTED') {
                                        ...
                                        let Disconnected = await client.channels.cache.get(player.textChannel).send({ embeds: [disconnectedEmbed] });
                                ...
                                client.warn(`Something unexpected happened with player ${player.options.guild}`);
                            }
                            player.setNowplayingMessage(client, null);
                        } catch (err) {
                            client.error(err);
    ...
    // TODO: Replace this with the functions from loadCommands()
    LoadCommands() {
        ...
        fs.readdir(SlashCommandsDirectory, (err, files) => {
            if (err) {
                ...
                files.forEach((file) => {
                    ...
                    if (!cmd || !cmd.run) {
                        return this.warn('Unable to load Command: ' + file.split('.')[0] + ", File doesn't have an valid command with run function");
                    }
            ...
            if (err) {
                ...
                files.forEach((file) => {
                    ...
                    if (!cmd.command || !cmd.run) {
                        return this.warn('Unable to load Command: ' + file.split('.')[0] + ", File doesn't have either command/run");
                    }
    ...
    createController(guild, player) {
        return new MessageActionRow().addComponents(
            new MessageButton().setStyle('DANGER').setCustomId(`controller:${guild}:Stop`).setEmoji('⏹️'),
            ...
            new MessageButton()
                .setStyle(player.trackRepeat ? 'SUCCESS' : player.queueRepeat ? 'SUCCESS' : 'DANGER')

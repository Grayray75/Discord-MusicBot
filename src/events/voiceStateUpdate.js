...
module.exports = async (client, oldState, newState) => {
    ...
    switch (stateChange.type) {
        case 'JOIN':
            onJoin(client, player, stateChange);
            break;
        case 'LEAVE':
            onLeave(client, player, stateChange);
            break;
    }
};

async function onJoin(client, player, stateChange) {
    if (player.get('autoPause') === true) {
        ...
        if (members === 1 && player.paused && members !== player.prevMembers) {
            ...
            let resumeMessage = await client.channels.cache.get(player.textChannel).send({ embeds: [playerResumed] });
...
async function onLeave(client, player, stateChange) {
    ...
    if (player.get('autoPause') === true && player.get('autoLeave') === false) {
        if (members === 0 && !player.paused && player.playing) {
            ...
            let playerPaused = new MessageEmbed().setColor(client.config.embedColor).setTitle(`Paused!`, client.config.iconURL).setFooter({
                ...
                setTimeout(async () => {
                    ...
                    if (members === 0 && player.state !== 'DISCONNECTED') {
                        let leftEmbed = new MessageEmbed()
                            ...
                            .setTimestamp();
                        let Disconnected = await client.channels.cache.get(player.textChannel).send({ embeds: [leftEmbed] });

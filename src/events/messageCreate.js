...
module.exports = async (client, message) => {
    ...
    const invite = `https://discord.com/oauth2/authorize?client_id=${client.config.clientId}&permissions=${client.config.inviteScopes.toString().replace(/,/g, '%20')}`;

    const buttons = new MessageActionRow().addComponents(
        new MessageButton().setStyle('LINK').setLabel('Invite me').setURL(invite),
        new MessageButton().setStyle('LINK').setLabel('Support server').setURL(`${client.config.supportServer}`)
    );

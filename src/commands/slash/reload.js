            ...
            try {
                ...
                fs.readdir(SlashCommandsDirectory, (err, files) => {
                    files.forEach((file) => {
                        ...
                        if (!cmd || !cmd.run) {
                            return client.warn('❌ Unable to load Command: ' + file.split('.')[0] + ", File doesn't have a valid command with run function");
                        }
                ...
                console.log(err);
                return interaction.reply({
                    embeds: [new MessageEmbed().setColor(client.config.embedColor).setDescription('An error has occured. For more details please check console.')],
                    ephemeral: true
                });
            }
        } else {
            return interaction.reply({
                embeds: [new MessageEmbed().setColor(client.config.embedColor).setDescription('You are not authorized to use this command!')],
                ephemeral: true
            });
        }
    });

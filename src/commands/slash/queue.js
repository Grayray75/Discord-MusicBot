...
const command = new SlashCommand()
    ...
    .setRun(async (client, interaction, options) => {
        ...
        if (!player) {
            return interaction.reply({
                embeds: [new MessageEmbed().setColor('RED').setDescription('There are no songs in the queue.')],
                ephemeral: true
            });
        }

        if (!player.playing) {
            const queueEmbed = new MessageEmbed().setColor(client.config.embedColor).setDescription("There's nothing playing.");
        ...
        if (!player.queue.size || player.queue.size === 0) {
            ...
            const queueEmbed = new MessageEmbed()
                ...
                .addFields(
                    {
                        name: 'Duration',
                        value: song.isStream ? `\`LIVE\`` : `\`${pms(player.position, { colonNotation: true })} / ${pms(player.queue.current.duration, { colonNotation: true })}\``,
                        inline: true
                    },
                    {
                        name: 'Volume',

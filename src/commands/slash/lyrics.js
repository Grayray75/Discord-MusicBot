...
const command = new SlashCommand()
    ...
    .setRun(async (client, interaction, options) => {
        await interaction.reply({
            embeds: [new MessageEmbed().setColor(client.config.embedColor).setDescription('🔎 | **Searching...**')]
        });
        ...
        if (client.manager) {
            ...
            return interaction.editReply({
                embeds: [new MessageEmbed().setColor('RED').setDescription('Lavalink node is not connected')]
            });
        }
        ...
        if (!args && !player) {
            return interaction.editReply({
                embeds: [new MessageEmbed().setColor('RED').setDescription("There's nothing playing")]
            });
        }
        ...
        lyricsApi
            ...
            .then(async (lyricsData) => {
                if (lyricsData.length !== 0) {
                    ...
                    const menu = new MessageActionRow().addComponents(
                        new MessageSelectMenu().setCustomId('choose-lyrics').setPlaceholder('Choose a song').addOptions(lyricsResults)
                    );
                    ...
                    let selectedLyrics = await interaction.editReply({
                        embeds: [
                            new MessageEmbed()
                                ...
                                .setDescription(`Here are some of the results I found for \`${query}\`. Please choose a song to display lyrics within \`30 seconds\`.`)
                        ],
                        components: [menu]
                    });
                        ...
                        if (interaction.isSelectMenu()) {
                            ...
                            lyricsApi.find(url).then((lyrics) => {
                                ...
                                const button = new MessageActionRow().addComponents(
                                    new MessageButton().setCustomId('tipsbutton').setLabel('Tips').setEmoji(`📌`).setStyle('SECONDARY'),
                                    new MessageButton().setLabel('Source').setURL(url).setStyle('LINK')
                                );
                                ...
                                if (lyricsText.length === 0) {
                                    lyricsEmbed.setDescription(`**Unfortunately we're not authorized to show these lyrics.**`).setFooter({
                    ...
                    collector.on('end', async (i) => {
                        if (i.size == 0) {
                            selectedLyrics.edit({
                                content: null,
                                embeds: [new MessageEmbed().setDescription(`No song is selected. You took too long to select a track.`).setColor(client.config.embedColor)],
                                components: []
                            });
                        }
                    });
                } else {
                    const button = new MessageActionRow().addComponents(new MessageButton().setEmoji(`📌`).setCustomId('tipsbutton').setLabel('Tips').setStyle('SECONDARY'));
                    return interaction.editReply({
                        embeds: [new MessageEmbed().setColor('RED').setDescription(`No results found for \`${query}\`!\nMake sure you typed in your search correctly.`)],
                        components: [button]
                    });
                }
            })
            .catch((err) => {
                console.error(err);
                return interaction.editReply({
                    embeds: [new MessageEmbed().setColor('RED').setDescription(`An unknown error has occured, please check your console.`)]
                });
            });

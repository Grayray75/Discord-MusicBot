...
const command = new SlashCommand()
    ...
    .setRun(async (client, interaction) => {
        ...
        helpEmbed.addFields({
            name: 'Credits',
            value:
                `Discord Music Bot Version: v${require('../../package.json').version}; Build: ${gitHash}` +
                '\n' +
        ...
        collector.on('collect', async (iter) => {
            ...
            tempArray.forEach((cmd) => {
                //console.log(cmd);
                helpEmbed.addFields({ name: cmd.name, value: cmd.description }).setFooter({ text: `Page ${pageNo + 1} / ${maxPages}` });
            });
            helpEmbed.addFields({
                name: 'Credits',
                value:

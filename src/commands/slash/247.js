...
const command = new SlashCommand()
    ...
    .setRun(async (client, interaction, options) => {
        ...
        if (!player) {
            return interaction.reply({
                embeds: [new MessageEmbed().setColor('RED').setDescription("There's nothing to play 24/7.")],
                ephemeral: true
            });
        }

        let twentyFourSevenEmbed = new MessageEmbed().setColor(client.config.embedColor);
        ...
        twentyFourSevenEmbed.setDescription(`**24/7 mode is** \`${!twentyFourSeven ? 'ON' : 'OFF'}\``).setFooter({
            text: `The bot will ${!twentyFourSeven ? 'now' : 'no longer'} stay connected to the voice channel 24/7.`
        });
        client.warn(
            `Player: ${player.options.guild} | [${colors.blue('24/7')}] has been [${colors.blue(!twentyFourSeven ? 'ENABLED' : 'DISABLED')}] in ${

    ...
    .setRun(async (client, interaction, options) => {
        ...
        try {
            ...
            if (res.loadType === 'LOAD_FAILED') {
                return interaction.reply({
                    embeds: [new MessageEmbed().setDescription('An error occured while searching for the song').setColor('RED')],
                    ephemeral: true
                });
            }
        } catch (err) {
        ...
        if (res.loadType == 'NO_MATCHES') {
            return interaction.reply({
                embeds: [new MessageEmbed().setDescription(`No results found for \`${search}\``).setColor('RED')],
                ephemeral: true
            });
        } else {
            let max = 10;
            ...
            const menus = new MessageActionRow().addComponents(new MessageSelectMenu().setCustomId('select').setPlaceholder('Select a song').addOptions(resultFromSearch));
            ...
            tracksCollector.on('end', async (i) => {
                if (i.size == 0) {
                    choosenTracks.edit({
                        content: null,
                        embeds: [new MessageEmbed().setDescription(`No track selected. You took too long to select a track.`).setColor(client.config.embedColor)],
                        components: []
                    });
                }
            });

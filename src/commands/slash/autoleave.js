...
const command = new SlashCommand()
    ...
    .setRun(async (client, interaction) => {
        ...
        autoLeaveEmbed.setDescription(`**Auto Leave is** \`${!autoLeave ? 'ON' : 'OFF'}\``).setFooter({
            text: `The player will ${!autoLeave ? 'now automatically' : 'not automatically'} leave when the voice channel is empty.`
        });
        client.warn(
            `Player: ${player.options.guild} | [${colors.blue('autoLeave')}] has been [${colors.blue(!autoLeave ? 'ENABLED' : 'DISABLED')}] in ${

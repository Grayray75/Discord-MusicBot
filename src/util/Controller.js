...
module.exports = async (client, interaction) => {
    ...
    if (!player) {
        await interaction.reply({
            embeds: [client.createEmbed('❌ | **There is no player to control in this server.**')]
        });
        setTimeout(() => {
    ...
    if (!interaction.member.voice.channel) {
        const joinEmbed = new MessageEmbed().setColor(client.config.embedColor).setDescription('❌ | **You must be in a voice channel to use this action!**');
    ...
    if (interaction.guild.members.me.voice.channel && !interaction.guild.members.me.voice.channel.equals(interaction.member.voice.channel)) {
        const sameEmbed = new MessageEmbed().setColor(client.config.embedColor).setDescription('❌ | **You must be in the same voice channel as me to use this action!**');
    ...
    if (property === 'Stop') {
        ...
        const msg = await interaction.channel.send({
            embeds: [client.createEmbed('⏹️ | **Successfully stopped the player**')]
        });
        setTimeout(() => {
            msg.delete();
        }, 5000);
    ...
    if (property === 'Replay') {
        ...
        if (!player.queue.previous || player.queue.previous === player.queue.current || player.queue.previous === player.queue[0]) {
            return interaction.reply({
                ephemeral: true,
                embeds: [new MessageEmbed().setColor('RED').setDescription(`There is no previous song played.`)]
            });
        }
    ...
    if (property === 'PlayAndPause') {
        if (!player || (!player.playing && player.queue.totalSize === 0)) {
            const msg = await interaction.channel.send({
                ephemeral: true,
                embeds: [new MessageEmbed().setColor('RED').setDescription('There is no song playing right now.')]
            });
            setTimeout(() => {
                msg.delete();
            }, 5000);
    ...
    if (property === 'Next') {
        ...
        if (player.queue[0] == undefined && (!autoQueue || autoQueue === false)) {
            return interaction.reply({
                ephemeral: true,
                embeds: [new MessageEmbed().setColor('RED').setDescription(`There is nothing after [${song.title}](${song.uri}) in the queue.`)]
            });
        } else player.stop();
        return interaction.deferUpdate;
    }
